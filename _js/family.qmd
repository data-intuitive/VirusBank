...
groupedAnnotatedViruses = annotatedViruses
    .map(virus => {
...
virusToTab = new Map(
  groupedAnnotatedViruses
    .map(virus => {
      if (typeof virus.group_abbreviation !== "undefined" && virus.group_abbreviation !== "") {
        console.log(">>> Virus " + virus.abbreviation + " belongs to group " + virus.group_abbreviation)
        return virus.group_abbreviation
      } else {
        console.log(">>> Virus " + virus.abbreviation)
        return virus.abbreviation
      }
    })
    .map( (virus,i) => [ virus, indexToTab(i, nrTabs) ] )
)

tabToVirus = new Map(Array.from(virusToTab, a => a.reverse()))

log = {
  console.log(rawTabs)
  console.log(virusToTab)
  console.log(tabToVirus)
  console.log(groupedAnnotatedViruses)
}
...
renderedTree = tree
  ...
  .style_nodes( (element, data) => {
    ...
    // Handle the grouped viruses
    const isGroup = (typeof information !== "undefined") && (typeof information.group_abbreviation !== "undefined") && (information.group_abbreviation !== "")
    const virus_pointer = (isGroup) ? information.group_abbreviation : virus_id 
    console.log(virus_id + " -> " + virus_pointer)
    ...
    if (information)
      element
        .on("mouseover", virusHoverIn(data, {... information, isGroup: isGroup}, inToolbox))
    ...
    inToolbox
      ? element.on('click', function() {
          ...
          const thisVirusInfo = document.getElementById(virusToTab.get(virus_pointer))  // either the virus_id or the group abbreviation
          thisVirusInfo.click();
...
virusHoverIn = (data, information, is_available) => () => {
  ...
  d3.select("#name").text((information.isGroup) ? information.group_virus_name : information.virus_name)
  if (is_available)
...
feedbackToolbox = d3.selectAll(".nav-item")
  .on('click', function(){ 
    ...
    clearToolbox();
    updateToolbox({ data: { name: tabToVirus.get(_id)}})
  })
