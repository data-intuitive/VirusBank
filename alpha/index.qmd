...
menu = [
    {
      title: (d) => { return "<a href='#transmission-and-epidemiology'>" + d.data.name + "</a>"},
      action: (elm, d, event) => {
...
// Initialize a container to contain the containers for the tree and 
html`
<div id="container">
  <div id="tree-container" style="position:relative"/>
  <div id="menu-container" class="card bg-light" style="position:relative;width:700px;">
    <!-- <div class="d3-context-menu d3-context-menu-theme"/> -->
  </div>
</div>
`

clearPopup = () => {
  const ct = d3.select('#menu-container')
  ct.selectAll("*").remove()
}
...
initPopup = () => {
  ...
  ctBody.append('p').attr("class", "card-text").text("...");
}
...
fillPopup = (d) => {
  ...
  ctBody.append('p').attr("class", "card-text").text("This is a strange virus... ");
}
...
s2 = d3.select(".menu-container").on('click', () => { d3.event. soptPropagation() })

renderedTree = new phylotree.phylotree(nwk)
  .render({
...
rt = renderedTree
  .style_nodes( (element, data) => {
    ...
    <!--   onClose: (data, event) => { -->
    <!--     // Add the div again to container the 'menu' -->
    <!--     d3.select("#menu-container").append("div").attr("class", "d3-context-menu d3-context-menu-theme") -->
    <!--   } -->
    <!-- })) -->
