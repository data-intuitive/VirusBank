...
<div class="container">
  <div class="grid">
    <div id="tree-container" class="tree-container g-col-7 text-center rounded"></div>
...
annotationsMap = Object.assign({}, ...(annotations.map(item => ({ [item.id]: { id: item.id, description: item.description, name: item.name, transmission: item.transmission } }) )))
...
fillPopup = (d) => {
  ...
  const card = ctBody.append('div').attr("class", "card-text");
    ...
    card.append("h4").text("Transmission and epidemiology")
    const this_div = card.append("div");

    console.log(annotationsMap[d.data.name])

    annotationsMap[d.data.name].transmission.map(el => this_div.append("p").text(el));
