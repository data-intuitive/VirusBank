...
<!-- json = await FileAttachment("copy.json").text() -->
<!-- tree = new phylotree.phylotree(json, { type: (input, options) => ({ json: input, error: null }) }) -->
...
rt = renderedTree
  .style_nodes(function(element, data) {
    ...
    element.on('contextmenu', (d) => {
      console.log("contextmenu triggered on " + d);
    ...
    element.on('mouseover', function() {
      ...
      // deselect all text elements by giving them the default class
      d3.selectAll("text").attr("selected", null).attr("class", "phylotree-node-text")

      // select the current element and update the class
      d3.select(this).select("text").text(data.data.name).attr("class", "phylotree-node-text-selected")

      d3.select('#mouseover-text').text(annotationsMap[data.data.name].name);
    })
  })
  .layout(true)
  .show()
...
d3.select(rt)
  ...
  .attr('class', "phylotree-node-text-info")
  .attr('transform', 'translate(20, 20)')
  .text('')

// This variable contains the selection from the table below
