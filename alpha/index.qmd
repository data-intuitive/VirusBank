  ...
  <div class="grid" style="--bs-gap: 0;">
    ...
    <div id="annotation-container" class="g-col-md-3 g-col-sm-12 no-gutter">
      <div class="container grid small" align="center" style="--bs-gap: 1.0rem;">
          <div class="g-col-md-12 g-col-sm-12 border bg-light card card-body tool-header opacity-100" id="virus">Please click on a virus</div>
          <div class="g-col-6 border card card-body tool opacity-50 text-wrap"  id="cpe">CPE reduction assay</div>
          <div class="g-col-6 border card card-body tool opacity-50" id="yield">Virus yield assay</div>
          <div class="g-col-6 border card card-body tool opacity-50" id="delay">Delay of treatment assay</div>
          <div class="g-col-6 border card card-body tool opacity-50" id="plaque">Plaque assay</div>
          <div class="g-col-6 border card card-body tool opacity-50" id="pcr">qRT-PCR</div>
          <div class="g-col-6 border card card-body tool opacity-50" id="reverse">Reverse genetics system</div>
          <div class="g-col-6 border card card-body tool opacity-50" id="model">
              <img class="card-img" src="/img/model-copy.png" style="height:100%;"/>
              <div class="card-img-overlay">
                Animal models
              </div>
          </div>
          <div class="g-col-6 border card card-body tool opacity-50"  id="reporter">
...
clearPopup = () => {
    d3.select("#cpe").attr("class",      "g-col-6 border round bg-dark card card-body tool opacity-25")
    d3.select("#model").attr("class",    "g-col-6 border round bg-dark card card-body tool opacity-25")
    d3.select("#yield").attr("class",    "g-col-6 border round bg-dark card card-body tool opacity-25")
    d3.select("#reverse").attr("class",  "g-col-6 border round bg-dark card card-body tool opacity-25")
    d3.select("#pcr").attr("class",      "g-col-6 border round bg-dark card card-body tool opacity-25")
    d3.select("#plaque").attr("class",   "g-col-6 border round bg-dark card card-body tool opacity-25")
    d3.select("#delay").attr("class",    "g-col-6 border round bg-dark card card-body tool opacity-25")
    d3.select("#reporter").attr("class", "g-col-6 border round bg-dark card card-body tool opacity-25")

  const virusInfo = document.getElementById("tabset-4-1-tab").parentNode.parentNode.parentNode
...
updateToolbox = (d) => {
  ...
  toolbox.map(el => {
    if (el.tool == "cpe") d3.select("#cpe").attr("class",           "g-col-6 border bg-dark card card-body tool opacity-100")
...
renderedTree = tree                   // class TreeRender
  ...
  .style_nodes( (element, data) => {
    ...
    if (availableInfo)
      element
        .on("mouseover", virus_hover_in(data, inToolbox))
...
<svg>
    <linearGradient id="Gradient">
      ...
      <stop stop_color="white" offset="50%" />
    </linearGradient>
</svg>
