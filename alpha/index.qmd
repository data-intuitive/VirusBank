...
function drawMenu(data, items, svgDomID, event) {
  var bbox = svgDomID.firstChild.getBBox();
  console.log(bbox);
  drawMenuDomElements(bbox.width+20, -bbox.height/2, menuItems, data, svgDomID);
  event.preventDefault();
}

function drawMenuDomElements(x, y, menuItems, d, svgDomID) {
  console.log("Drawing menu on" + svgDomID);
...
renderedTree.style_nodes(function(element, data) {
    ...
    element.on('mouseover', function(event, d) {
      ...
      drawMenu(data, menuItems, this, event);
    });
  })

rt = renderedTree.show();
