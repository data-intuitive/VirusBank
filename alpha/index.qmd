...
<style type="text/css">
  ...
  // temp pasted the complete CSS file here
  .d3-context-menu {
    position: absolute;
    min-width: 700px;
    z-index: 1200;
...
menu = [
    {
      title: (d) => { console.log(d); return d.data.name },
      action: (d) => {
...
renderedTree = tree
  .render({
    ...
    'is-radial': true,
    'selectable': true,  // make nodes and branches not selectable
    'collapsible': true,
    'transitions' : true,
    'show-menu': true,
    'show-scale': false,
...
renderedTree
  .style_nodes( (element, data) => {
    element.on('click', d3.contextMenu(menu, {
      onClose: (data, event) => {
...
html`<div id="menu-container"><div class="d3-context-menu d3-context-menu-theme"/></div>`
