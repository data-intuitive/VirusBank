    ...
    if (isLeaf) {
      ...
      if (virusOfInterest(node.data.name)) {
        ...
        try {
          ...
          console.error("Could not remove node: %s. Please check that not \
            all virusses are being removed from the tree and that the virus \
            IDs in the family excel file match with the newick file.", virus)
        }
      }
    } 
  })
