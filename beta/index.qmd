...
import { annotations2 } from "./ojs_data.qmd"
...
updateToolbox = (d) => {
  ...
  const this_toolbox = availableTools(d.data.name)

  d3.select("#virus").text(annotationsMap[d.data.name].name)
...
renderedTree = tree
  .traverse_and_compute(node => {
    ...
    if (isLeaf) {
      const information = annotations2.filter(obj => obj["Virus ID"] == node.data.name)[0]
      // console.log(information)
      // console.log("Leaf node, check if needs to be shown")
      if (information["Virus of Interest"] == "Yes") {
        // console.log("Virus of interest")
        const current_name = node.data.name
        const new_name = information.Abbreviation
        // console.log("current: " + current_name)
      ...
      // console.log("no leaf node, remove if necesary")
    }
  })
  .render({
